...
impl<'input> JsonError<'input> {
    ...
    pub(crate) fn new_unexpected(
        ...
        if got == Token::EOF {
            JsonError::new(JsonErrorKind::UnexpectedEof(wanted), input, span)
        } else {
            JsonError::new(JsonErrorKind::UnexpectedToken { got, wanted }, input, span)
        }
    }
...
impl core::fmt::Display for JsonErrorMessage<'_> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match &self.0.kind {
            JsonErrorKind::UnexpectedEof(msg) => {
                write!(f, "Unexpected end of file: wanted {}", msg.red())
            }
