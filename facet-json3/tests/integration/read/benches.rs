...
#[test]
fn bench_nested_facet_json3_serialize() {
    let data = create_nested_data();

    facet_json3::to_string(&data);
}

#[test]
fn bench_nested_facet_json3_deserialize() {
    let data = create_nested_data();
    let json_string = facet_json3::to_string(&data);

    let _: Vec<Nested15> = facet_json3::from_str(&json_string).unwrap();
}

// Wide benchmark functions

#[test]
fn bench_wide_facet_json3_serialize() {
    let data = create_wide();
    facet_json3::to_string(&data);
}

#[test]
fn bench_wide_facet_json3_deserialize() {
    let data = create_wide();
    let json_string = facet_json3::to_string(&data);

    let _: Wide = facet_json3::from_str(&json_string).unwrap();
}
