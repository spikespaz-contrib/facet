...
fn test_eof_errors() {
    ...
    assert!(matches!(err.kind, JsonErrorKind::UnexpectedEof("string")));
    ...
    assert!(matches!(
        err.kind,
        JsonErrorKind::UnexpectedEof("object key or closing brace")
    ));

    // Test object with EOF after key
    let result = from_str::<SimpleObject>("{\"key\"");
    let err = result.unwrap_err();
    assert!(matches!(err.kind, JsonErrorKind::UnexpectedEof("colon")));
