...
fn test_field_rename_roundtrip() {
    ...
    let json = facet_json3::to_string(&original);
...
fn test_field_rename_with_symbol_chars_name() {
    ...
    let json = facet_json3::to_string(&test_struct);
    assert_eq!(json, r#"{"@#$%^&":"special value"}"#);

    let roundtrip: SpecialCharsName = facet_json3::from_str(&json).unwrap();
...
fn test_field_rename_missing_required_error() {
    ...
    let result = from_str::<Required>(json);
    let e = result.unwrap_err();
    assert!(matches!(
        e.kind,
        JsonErrorKind::MissingField(f) if f == "original_field"
    ));
    #[cfg(not(miri))]
